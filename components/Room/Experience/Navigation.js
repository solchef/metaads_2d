...
export default class Navigation {
  ...
  setView() {
    ...
    this.view.spherical.limits.radius = { min: 50, max: 70 }
    this.view.spherical.limits.phi = { min: 0.1, max: Math.PI * 0.5 }
    this.view.spherical.limits.theta = { min: -Math.PI * 0.5, max: 2 }
    ...
    this.view.move2 = (number) => {
      if (this.view.spherical.value.theta <= -0.9 || Math.sign(number) === 1) {
        if (
          this.view.spherical.value.theta >= -1.1 ||
          Math.sign(number) === -1
        ) {
          this.view.drag.delta.x += number - this.view.drag.previous.x / 5
          this.view.drag.delta.y += number + this.view.drag.previous.y / 2
          this.view.drag.previous.x = number
          this.view.drag.previous.y = number
        }
      }
    }
    ...
    this.view.onWheel = (_event) => {
      ...
      this.view.move2(normalized.pixelY * 0.1)
    }
